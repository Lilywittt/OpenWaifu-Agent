...
from runtime_layout import build_run_bundle, initialize_run_bundle, sanitize_segment
...
def create_sample_bundle(sample_root: Path, sample_id: int):
    ensure_dir(sample_root)
    bundle = build_run_bundle(
        sample_root,
        run_id=f"{sample_root.parent.name}_sample{sample_id:02d}",
    )
    initialize_run_bundle(bundle)
    return bundle
