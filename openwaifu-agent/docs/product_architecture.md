...
`QQ` 是消息入口，负责消息触发和回传；`public workbench` 是 richer 的交互式网页入口，允许选择起点、终点并查看阶段产物。因为这条 richer 业务和私有测试工作台共享同一条生成链，所以两者共用 `src/workbench/` 这一套共享内核。
...
`public workbench` 和私有工作台共用同一条任务链，差别体现在权限和身份。私有模式覆盖目录审阅、收藏、删目录、cleanup 这些系统级能力；公共模式覆盖发起任务、查看状态、查看阶段产物、查看结果，并且所有历史和详情都按 `ownerId` 做隔离。`ownerId` 由 `Cloudflare Access` 登录身份解析而来。
