...
def build_developer_input_received_text(scene_premise: str) -> str:
...
def build_status_text_from_payload(payload: dict[str, Any], *, mode: str | None = None) -> str:
    ...
    if status == "running":
        if run_id:
            return f"当前正在生图中，处于{mode_label}。runId: {run_id}。阶段：{stage}"
        return f"当前正在生图中，处于{mode_label}。阶段：{stage}"
